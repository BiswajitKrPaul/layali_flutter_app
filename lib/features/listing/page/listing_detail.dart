...
import 'package:layali_flutter_app/features/listing/cubits/listing_detail_cubit/listing_detail_cubit.dart';
...
@RoutePage()
class ListingDetail extends StatelessWidget implements AutoRouteWrapper {
  const ListingDetail({required this.id, super.key});
  final String id;

  @override
  Widget wrappedRoute(BuildContext context) {
    return BlocProvider(
      create: (context) => ListingDetailCubit()..getPropertyDetail(id),
      child: this,
    );
  }
    ...
    return Scaffold(
      body: BlocBuilder<ListingDetailCubit, ListingDetailState>(
        builder: (context, state) {
          ...
          if (state.hasError) {
            return Column(
              children: [
                ...
                const Text('Not able to fetch property details'),
                const Gap(8),
                FilledButton(
                  onPressed: () {
                    context.read<ListingDetailCubit>().getPropertyDetail(id);
                  },
          ...
          return Stack(
            children: [
              ImageBehindListScreen(property: state.propertyDetailModel!),
              Align(
                    ...
                    boxShadow: [
                      BoxShadow(
                        ...
                        offset: Offset(
                          0,
                          -4,
                        ), // Negative Y offset for top shadow
                      ),
                    ],
                  ),
                  child: Column(
                    ...
                    children: [
                      SizedBox(
                        ...
                        child: BottomCartWidget(
                          property: state.propertyDetailModel!,
                        ),
                      ),
                ...
                RichText(
                  ...
                  text: TextSpan(
                    ...
                    children: [
                      ...
                      TextSpan(
                        text:
                            '${widget.property.bedrooms} ${Intl.plural(widget.property.bedrooms, one: "bedroom", other: "bedrooms")}',
                      ),
                      const TextSpan(text: ' • '),
                      TextSpan(
                        text:
                            '${widget.property.beds} ${Intl.plural(widget.property.beds, one: "bed", other: "beds")}',
                      ),
                      const TextSpan(text: ' • '),
                      TextSpan(
                        text:
                            '${widget.property.bathrooms} ${Intl.plural(widget.property.bathrooms, one: "bathroom", other: "bathrooms")}',
                      ),
                    ],
                  ),
                ),
